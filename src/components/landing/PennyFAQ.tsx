  ...
  return (
    <>
      ...
      <div className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 z-50">
        {/* Label */}
        {showLabel && !isOpen && (
          <div className="absolute bottom-16 sm:bottom-20 right-0 bg-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full shadow-lg border border-gray-200 whitespace-nowrap animate-in fade-in slide-in-from-bottom-2 duration-300">
            <p className="text-xs sm:text-sm font-medium text-gray-900">Talk to Penny</p>
          </div>
        )}
        ...
        <button
          ...
          className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 transition-all duration-300 hover:scale-110"
      ...
      {/* Chat Popup */}
      {isOpen && (
        <div className="fixed inset-x-4 bottom-20 sm:bottom-24 sm:right-6 sm:left-auto sm:w-[380px] h-[500px] sm:h-[600px] bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 flex flex-col animate-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
          ...
          {messages.length <= 1 && (
            <div className="p-3 sm:p-4 border-b border-gray-100 bg-gray-50">
              <p className="text-[10px] sm:text-xs font-medium text-gray-600 mb-2">Quick questions:</p>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {faqData.quickPrompts.slice(0, 4).map((prompt) => (
                  <button
                    key={prompt.id}
                    onClick={() => handleQuickPrompt(prompt.text)}
                    className="text-[10px] sm:text-xs px-2 sm:px-3 py-1 sm:py-1.5 bg-white hover:bg-[#0044FF] hover:text-white border border-gray-200 rounded-full transition-all duration-200"
                  >
                    {prompt.text}
                  </button>
                ))}
          ...
          <div className="p-3 sm:p-4 border-t border-gray-200">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#0044FF] focus:border-transparent text-xs sm:text-sm"
              />
              <Button
                onClick={() => handleSendMessage()}
                disabled={!input.trim()}
                size="icon"
                className="rounded-full w-9 h-9 sm:w-10 sm:h-10 bg-[#0044FF] hover:bg-[#0033CC] disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              >
